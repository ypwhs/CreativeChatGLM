...
""")
    with gr.Row():
        ...
        with gr.Column(scale=1):
            with gr.Row():
                max_length = gr.Slider(32, 8192, value=8192, step=1.0, label="Maximum length", interactive=True)
                top_p = gr.Slider(0.01, 1, value=0.7, step=0.01, label="Top P", interactive=True)
                temperature = gr.Slider(0.01, 1.7, value=4.99, step=0.01, label="Temperature", interactive=True)
            with gr.Row():
